...
func StartCrawler() {
	...
	service, err := selenium.NewChromeDriverService(`D:\下载\chromedriver_win32\chromedriver.exe`, 9515, opts...)
	if err != nil {
	...
	csvFile, err := os.Create(pathxing)
	...
	for {
		...
		for i := 0; i < len(lists); i++ {
			...
			modelElem, err := webDriver.FindElement(selenium.ByXPATH, "/html/body/div[5]/div[2]/h3")
			var model string
			if err != nil {
				log.Println(err)
				model = "暂无"
			...
			storeElem, err := webDriver.FindElement(selenium.ByXPATH, "/html/body/div[10]/div/div[1]/div[1]/div/h4")
			var store string
			if err != nil {
				log.Println(err)
				store = "暂无"
...
func sendResult(fileName string) {
	...
	dialer := &gomail.Dialer{
		Host:     "smtp.163.com",
		Port:     25,
		Username: "26437542@qq.com",    // 替换自己的邮箱地址
		Password: "yangx6031", // 自定义smtp服务器密码
		SSL:      false,
	}
